...
FILES = {
    ...
    / "api"
    / "routers"
    / "paper_final_assurance_qualification_gate_history_runtime_dashboard.py":
        BACKEND
        / "app"
        / "api"
        / "routers"
    ...
    PAYLOAD
    / "tests"
    / "test_final_paper_assurance_qualification_gate_history_runtime_dashboard.py":
        BACKEND
        / "tests"
        / "test_final_paper_assurance_qualification_gate_history_runtime_dashboard.py",
}
...
def patch_application(
    ...
    if IMPORT_LINE not in text:
        ...
        inserted = (
            insert_after_import_statement(
                lines,
                anchor=(
                    "paper_final_assurance_qualification_"
                    "gate_history_runtime_router"
                ),
        ...
        if not inserted:
            raise RuntimeError(
                "Import da Fase 8AS não encontrado."
            )
    ...
    entry = (
        "        "
        "paper_final_assurance_qualification_"
        "gate_history_runtime_dashboard_router,"
    )
    ...
    if entry not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Router da Fase 8AS não encontrado na tupla."
            )
