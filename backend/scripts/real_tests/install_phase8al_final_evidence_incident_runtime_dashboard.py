...
FILES = {
    ...
    / "api"
    / "routers"
    / "paper_final_validation_evidence_incident_runtime_dashboard.py":
        BACKEND
        / "app"
        / "api"
        / "routers"
    ...
    PAYLOAD
    / "tests"
    / "test_final_paper_validation_evidence_incident_runtime_dashboard.py":
        BACKEND
        / "tests"
        / "test_final_paper_validation_evidence_incident_runtime_dashboard.py",
}
...
def patch_application(
    ...
    if IMPORT_LINE not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Import da Fase 8AK não encontrado."
            )
    ...
    if entry not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Router da Fase 8AK não encontrado na tupla."
            )
