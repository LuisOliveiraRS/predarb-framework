...
FILES = {
    ...
    / "app"
    / "paper"
    / "final_paper_validation_evidence_incidents.py":
        BACKEND
        / "app"
        / "paper"
        / "final_paper_validation_evidence_incidents.py",
    ...
    / "api"
    / "routers"
    / "paper_final_validation_evidence_incidents.py":
        BACKEND
        / "app"
        / "api"
        / "routers"
    ...
    PAYLOAD
    / "tests"
    / "test_final_paper_validation_evidence_incidents.py":
        BACKEND
        / "tests"
        / "test_final_paper_validation_evidence_incidents.py",
}
...
IMPORT_LINE = (
    ...
    "paper_final_validation_evidence_incidents "
    "import router as "
...
def patch_application(backup_dir: Path) -> None:
    ...
    if IMPORT_LINE not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Import da Fase 8AH não encontrado."
            )
    ...
    if entry not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Router da Fase 8AH não encontrado na tupla."
            )
...
def main() -> None:
    ...
    backup_dir = BACKEND / "backups" / "phase8ai" / stamp
    ...
    print()
    print("FASE 8AI INSTALADA E VALIDADA")
