...
FILES = {
    ...
    / "api"
    / "routers"
    / "paper_final_validation_evidence_incidents_dashboard.py":
        BACKEND
        / "app"
        / "api"
        / "routers"
    ...
    PAYLOAD
    / "tests"
    / "test_final_paper_validation_evidence_incidents_dashboard.py":
        BACKEND
        / "tests"
        / "test_final_paper_validation_evidence_incidents_dashboard.py",
}
...
def patch_application(
    ...
    if IMPORT_LINE not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Import da Fase 8AI não encontrado."
            )
    ...
    if entry not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Router da Fase 8AI não encontrado na tupla."
            )
