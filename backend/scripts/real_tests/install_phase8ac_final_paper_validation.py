...
FILES = {
    ...
    PAYLOAD
    / "tests"
    / "test_final_paper_validation.py":
        BACKEND
        / "tests"
        / "test_final_paper_validation.py",
}
...
def patch_application(
    ...
    if IMPORT_LINE not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Imports das fases 8AA/8AB não foram encontrados."
            )
    ...
    if entry not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Routers das fases 8AA/8AB não foram encontrados na tupla."
            )
