...
FILES = {
    ...
    / "api"
    / "routers"
    / "paper_performance_incident_dashboard.py":
        BACKEND
        / "app"
        / "api"
        / "routers"
    ...
    PAYLOAD
    / "tests"
    / "test_paper_performance_incident_dashboard.py":
        BACKEND
        / "tests"
        / "test_paper_performance_incident_dashboard.py",
}
...
def patch_application(
    ...
    if IMPORT_LINE not in text:
        ...
        for index, line in enumerate(lines):
            if (
                ...
                and "paper_performance_incidents_router"
                in line
            ):
                lines.insert(
                    index + 1,
        ...
        if not inserted:
            raise RuntimeError(
                "Import da Fase 8E não encontrado."
            )
    ...
    if entry not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Router da Fase 8E não encontrado "
                "na tupla de routers."
            )
