...
FILES = {
    ...
    / "api"
    / "routers"
    / "paper_certification_assurance_gate_history_runtime_dashboard.py":
        BACKEND
        / "app"
        / "api"
        / "routers"
    ...
    PAYLOAD
    / "tests"
    / "test_paper_certification_assurance_gate_history_runtime_dashboard.py":
        BACKEND
        / "tests"
        / "test_paper_certification_assurance_gate_history_runtime_dashboard.py",
}
...
def patch_application(
    ...
    if IMPORT_LINE not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Import da Fase 8AA não encontrado."
            )
    ...
    if entry not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Router da Fase 8AA não encontrado na tupla."
            )
