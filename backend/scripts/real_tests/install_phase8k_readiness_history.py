...
FILES = {
    ...
    PAYLOAD
    / "tests"
    / "test_paper_readiness_history.py":
        BACKEND
        / "tests"
        / "test_paper_readiness_history.py",
}
...
def patch_application(
    ...
    if IMPORT_LINE not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Import da Fase 8J não encontrado."
            )
    ...
    if entry not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Router da Fase 8J não encontrado "
                "na tupla de routers."
            )
