...
FILES = {
    ...
    / "api"
    / "routers"
    / "paper_performance_incident_runtime_dashboard.py":
        BACKEND
        / "app"
        / "api"
        / "routers"
    ...
    PAYLOAD
    / "tests"
    / "test_paper_performance_incident_runtime_dashboard.py":
        BACKEND
        / "tests"
        / "test_paper_performance_incident_runtime_dashboard.py",
}
...
def patch_application(
    ...
    if IMPORT_LINE not in text:
        ...
        for index, line in enumerate(lines):
            if (
                ...
                and "paper_performance_incident_runtime_router"
                in line
            ):
                lines.insert(
                    index + 1,
        ...
        if not inserted:
            raise RuntimeError(
                "Import da Fase 8G não encontrado."
            )
    ...
    if entry not in text:
        ...
        if not inserted:
            raise RuntimeError(
                "Router da Fase 8G não encontrado "
                "na tupla de routers."
            )
