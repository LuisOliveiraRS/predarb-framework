...
def test_capture_once_persists_monitor_snapshot():
    storage = []
    ...
    assert runtime.failed_cycles == 0
...
def test_runtime_records_failure_without_safety_change():
    runtime = runtime_with(
        [],
...
def test_reset_statistics_requires_stopped_runtime():
    async def scenario():
        runtime = runtime_with([])

        await runtime.capture_once()
        reset = await runtime.reset_statistics()
...
def test_runtime_write_routes_are_post_only():
    from app.api.routers.paper_performance_incident_runtime import (
        router,
    )
