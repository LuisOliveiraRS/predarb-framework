...
class AssuranceStub:
    ...
    def evaluate(self):
        payload = {
            "status": self.status,
            "assured": self.status == "ASSURED",
            "scope": "PAPER_ASSURANCE_ONLY",
            "assurance_score": (
                100
                if self.status == "ASSURED"
                else 75
            ),
            "summary": {
                "active_incidents": 0,
...
def runtime_with(
    ...
    *,
    ...
    return FinalPaperAssuranceHistoryRuntime(
        assurance_provider=(
            assurance
            or AssuranceStub()
        ),
...
def test_manual_cycle_captures_assurance():
    storage = []
    ...
    assert storage == ["ASSURED"]
    assert runtime.assured_cycles == 1
...
def test_status_counters_are_updated():
    ...
    assert runtime.warning_cycles == 1
    assert runtime.assured_cycles == 0
...
def test_runtime_rejects_unsafe_assurance():
    runtime = runtime_with(
        [],
        assurance=AssuranceStub(
            unsafe=True
...
def test_application_registers_runtime_routes():
    ...
    from app.api.routers.paper_final_assurance_history_runtime import (
        router,
    )
    ...
    required = {
        "/paper/final-assurance/history-runtime/health",
        "/paper/final-assurance/history-runtime/status",
        "/paper/final-assurance/history-runtime/last-cycle",
        "/paper/final-assurance/history-runtime/cycle",
        "/paper/final-assurance/history-runtime/start",
        "/paper/final-assurance/history-runtime/stop",
        "/paper/final-assurance/history-runtime/reset-statistics",
    }
