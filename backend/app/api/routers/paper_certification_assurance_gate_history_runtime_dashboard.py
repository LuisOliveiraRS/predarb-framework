...
<head>
  ...
  >
  <title>PredArb | Runtime do Histórico do Gate</title>
...
<body>
  <main>
    <header>
      <div>
        <h1>Runtime do Histórico do Gate</h1>
        <p class="muted">
          Captura periódica e controlada das avaliações de qualificação.
        </p>
      </div>
      ...
      <div class="actions">
        ...
        <a
          class="button"
          href="/paper/certification/assurance/gate/history/dashboard"
        >
          Histórico
        </a>
      </div>
  ...
  <script>
    ...
    async function startRuntime() {
      ...
      await post(
        "/paper/certification/assurance/"
        + "gate/history-runtime/start"
        + "?confirm=START-PAPER-ASSURANCE-GATE-HISTORY-RUNTIME"
    ...
    async function stopRuntime() {
      const confirmed = window.confirm(
        "Parar o runtime do histórico do gate?"
      );
      ...
      await post(
        "/paper/certification/assurance/"
        + "gate/history-runtime/stop"
        + "?confirm=STOP-PAPER-ASSURANCE-GATE-HISTORY-RUNTIME"
      );
    ...
    async function runCycle() {
      const confirmed = window.confirm(
        "Registrar agora a avaliação do Gate de Qualificação Paper?"
      );
    ...
    async function resetStatistics() {
      ...
      await post(
        "/paper/certification/assurance/"
        + "gate/history-runtime/reset-statistics"
        + "?confirm=RESET-PAPER-ASSURANCE-GATE-HISTORY-RUNTIME"
      );
...
async def assurance_gate_history_runtime_dashboard():
    return HTMLResponse(
...
@router.get("/snapshot")
async def assurance_gate_history_runtime_dashboard_snapshot():
    runtime = (
        paper_assurance_gate_history_runtime
        .status()
    )
