...
router = APIRouter(
    ...
    tags=[
        "paper-final-assurance-history-runtime"
    ],
)
...
@router.get("/health")
async def final_assurance_history_runtime_health():
    return {
        "status": "healthy",
        "runtime": (
            final_paper_assurance_history_runtime
...
@router.get("/status")
async def final_assurance_history_runtime_status():
...
async def final_assurance_history_runtime_last_cycle():
    status = (
        final_paper_assurance_history_runtime
        .status()
    )
...
@router.post("/cycle")
async def final_assurance_history_runtime_cycle(
    confirm: str = Query(...),
):
    if (
        confirm
    ...
    try:
        return _safe_response(
            await final_paper_assurance_history_runtime
            .capture_once()
        )
...
@router.post("/start")
async def final_assurance_history_runtime_start(
    confirm: str = Query(...),
...
@router.post("/stop")
async def final_assurance_history_runtime_stop(
    confirm: str = Query(...),
):
    if (
        confirm
...
async def final_assurance_history_runtime_reset_statistics(
    confirm: str = Query(...),
):
    if (
        confirm
