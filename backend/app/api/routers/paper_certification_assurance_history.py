...
router = APIRouter(
    prefix=(
        "/paper/certification/assurance/history"
    ),
    tags=[
        "paper-certification-assurance-history"
    ],
...
<head>
  ...
  >
  <title>PredArb | Histórico da Garantia Paper</title>
...
<body>
  <main>
    <header>
      <div>
        <h1>Histórico da Garantia Paper</h1>
        <p class="muted">
          Evolução temporal do Centro de Garantia e seus estados.
        </p>
      </div>
      ...
      <div class="actions">
        ...
        <a
          class="button"
          href="/paper/certification/assurance/history/export.csv"
        >
          Exportar CSV
        </a>
    ...
    <section class="panel">
      <h2>Evolução do score consolidado</h2>
      ...
      <svg
        ...
        role="img"
        aria-label="Histórico do score de garantia"
      ></svg>
    </section>

    <section class="panel">
      <h2>Snapshots registrados</h2>
  ...
  <script>
    ...
    function render(payload) {
      ...
      document.getElementById(
        ...
        card(
          ...
          integer.format(
            Number(
              summary.longest_assured_streak
              || 0
            )
          )
        ),
    ...
    async function refresh() {
      ...
      try {
        ...
        document.getElementById(
          ...
          + " | Histórico limitado ao ambiente Paper"
        );
      } catch (error) {
        document.getElementById(
          "lastUpdate"
    ...
    async function capture() {
      const confirmed = window.confirm(
        "Registrar o snapshot atual do Centro de Garantia Paper?"
      );
      ...
      try {
        const response = await fetch(
          "/paper/certification/assurance/"
          + "history/capture"
          + "?confirm=CAPTURE-PAPER-CERTIFICATION-ASSURANCE",
          {
            method: "POST"
          }
        );
        ...
        button.disabled = false;
        button.textContent = "Registrar snapshot";
      }
    }
...
@router.get("/health")
async def assurance_history_health():
...
@router.get("/summary")
async def assurance_history_summary():
    return _history().summary()


@router.get("/latest")
async def assurance_history_latest():
    return {
...
@router.get("/entries")
async def assurance_history_entries(
    limit: int = Query(
        default=100,
        ge=1,
        le=5000,
...
@router.get("/snapshot")
async def assurance_history_snapshot(
    limit: int = Query(
        default=250,
        ge=1,
        le=5000,
...
@router.post("/capture")
async def assurance_history_capture(
    confirm: str = Query(...),
):
    if (
        confirm
...
async def assurance_history_dashboard():
    return HTMLResponse(
...
async def assurance_history_export_csv(
    limit: int = Query(
        default=5000,
        ge=1,
        le=5000,
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                "attachment; "
                'filename="predarb-paper-assurance-history.csv"'
            ),
            "Cache-Control": "no-store",
            "X-PredArb-Live-Authorization": "false",
            "X-PredArb-Financial-Execution": "false",
