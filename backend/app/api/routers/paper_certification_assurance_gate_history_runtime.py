...
router = APIRouter(
    ...
    tags=[
        "paper-certification-assurance-gate-history-runtime"
    ],
)
...
@router.get("/health")
async def assurance_gate_history_runtime_health():
    status = (
        paper_assurance_gate_history_runtime
        .status()
    )
...
@router.get("/status")
async def assurance_gate_history_runtime_status():
...
async def assurance_gate_history_runtime_last_cycle():
    status = (
        paper_assurance_gate_history_runtime
        .status()
    )
...
@router.post("/cycle")
async def assurance_gate_history_runtime_cycle(
    confirm: str = Query(...),
):
    if (
        confirm
    ...
    try:
        return _safe_response(
            await paper_assurance_gate_history_runtime
            .capture_once()
        )
...
@router.post("/start")
async def assurance_gate_history_runtime_start(
    confirm: str = Query(...),
...
@router.post("/stop")
async def assurance_gate_history_runtime_stop(
    confirm: str = Query(...),
):
    if (
        confirm
...
async def assurance_gate_history_runtime_reset_statistics(
    confirm: str = Query(...),
):
    if (
        confirm
