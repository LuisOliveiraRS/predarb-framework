...
router = APIRouter(
    prefix="/paper/final-assurance/history",
    tags=["paper-final-operational-assurance-history"],
)
...
<head>
  ...
  >
  <title>PredArb | Histórico da Garantia Final Paper</title>
...
<body>
  <main>
    <header>
      <div>
        <h1>Histórico da Garantia Final Paper</h1>
        <p class="muted">
          Evolução persistente do estado operacional consolidado.
        </p>
      </div>
      ...
      <div class="actions">
        ...
        <a
          class="button"
          href="/paper/final-assurance/history/export.csv"
        >
          Exportar CSV
        </a>
        ...
        <a
          class="button"
          href="/paper/final-assurance/history/export.json"
        >
          Exportar JSON
        </a>
    ...
    <section class="panel">
      ...
      <svg
        ...
        role="img"
        aria-label="Evolução do score de garantia"
      ></svg>
    </section>
  ...
  <script>
    ...
    function render(payload) {
      ...
      document.getElementById(
        ...
        card(
          ...
          integer.format(
            Number(
              summary.longest_assured_streak || 0
            )
          )
        ),
        card(
          ...
          return `
            <tr>
              ...
              <td>
                ${integer.format(
                  Number(
                    details.active_critical_incidents || 0
                  )
                )}
              </td>
              <td>
    ...
    async function refresh() {
      ...
      try {
        const response = await fetch(
          "/paper/final-assurance/history/snapshot?limit=500",
          {
            cache: "no-store"
          }
        );
    ...
    async function capture() {
      const confirmed = window.confirm(
        "Registrar a avaliação atual da Garantia Operacional Final Paper?"
      );
...
def _history() -> FinalPaperOperationalAssuranceHistory:
    return (
        FinalPaperOperationalAssuranceHistory()
    )
...
@router.get("/health")
async def final_assurance_history_health():
...
@router.get("/summary")
async def final_assurance_history_summary():
    return _history().summary()


@router.get("/latest")
async def final_assurance_history_latest():
    return {
...
@router.get("/entries")
async def final_assurance_history_entries(
    limit: int = Query(
        default=500,
        ge=1,
        le=5000,
...
@router.get("/snapshot")
async def final_assurance_history_snapshot(
    limit: int = Query(
        default=500,
        ge=1,
        le=5000,
...
@router.post("/capture")
async def final_assurance_history_capture(
    confirm: str = Query(...),
):
    if (
        confirm
...
async def final_assurance_history_dashboard():
    return HTMLResponse(
...
async def final_assurance_history_export_csv(
    limit: int = Query(
        default=5000,
        ge=1,
        le=5000,
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                "attachment; "
                'filename="predarb-final-paper-assurance-history.csv"'
            ),
            "Cache-Control": "no-store",
            "X-PredArb-Live-Authorization": "false",
            "X-PredArb-Financial-Execution": "false",
...
async def final_assurance_history_export_json():
    history = _history()
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                "attachment; "
                'filename="predarb-final-paper-assurance-history.json"'
            ),
            "Cache-Control": "no-store",
            "X-PredArb-Live-Authorization": "false",
            "X-PredArb-Financial-Execution": "false",
