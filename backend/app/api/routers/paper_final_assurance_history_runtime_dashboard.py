...
router = APIRouter(
    prefix="/paper/final-assurance/history-runtime",
    tags=["paper-final-assurance-history-runtime-dashboard"],
)
...
<head>
  ...
  >
  <title>PredArb | Runtime do Histórico da Garantia Final</title>
...
<body>
  <main>
    <header>
      <div>
        <h1>Runtime do Histórico da Garantia Final</h1>
        <p class="muted">
          Captura controlada da Garantia Operacional Final no histórico Paper.
        </p>
      </div>
      ...
      <div class="actions">
        ...
        <a
          class="button"
          href="/paper/final-assurance/history/dashboard"
        >
          Histórico
        </a>
      </div>
    ...
    <section class="badges">
      ...
      <span class="badge">
        Captura somente a garantia Paper
      </span>
      <span class="badge">
  ...
  <script>
    ...
    function render(payload) {
      ...
      document.getElementById(
        ...
        card(
          "Garantia atual",
          assurance.status || "-",
          css(assurance.status)
        ),
        card(
          "Score atual",
          Number(
            assurance.assurance_score || 0
          ).toFixed(0)
        ),
        card(
          "Ciclos",
        ...
        card(
          ...
          integer.format(
            Number(
              history.longest_assured_streak || 0
            )
          )
        ),
        card(
        ...
        card(
          "Ciclos ASSURED",
          integer.format(
            Number(runtime.assured_cycles || 0)
          )
        ),
        card(
          "Ciclos WARNING",
          integer.format(
            Number(runtime.warning_cycles || 0)
          )
        ),
        card(
          "Ciclos BLOCKED",
    ...
    async function startRuntime() {
      const confirmed = window.confirm(
        "Iniciar o runtime do histórico da Garantia Final Paper?"
      );
      ...
      await post(
        ...
        + "?confirm=START-FINAL-PAPER-ASSURANCE-HISTORY-RUNTIME"
    ...
    async function stopRuntime() {
      const confirmed = window.confirm(
        "Parar o runtime do histórico da Garantia Final Paper?"
      );
      ...
      await post(
        ...
        + "?confirm=STOP-FINAL-PAPER-ASSURANCE-HISTORY-RUNTIME"
      );
    ...
    async function runCycle() {
      const confirmed = window.confirm(
        "Capturar agora a Garantia Operacional Final Paper?"
      );
    ...
    async function resetStatistics() {
      ...
      await post(
        ...
        + "?confirm=RESET-FINAL-PAPER-ASSURANCE-HISTORY-RUNTIME"
      );
...
async def final_assurance_history_runtime_dashboard():
    return HTMLResponse(
...
@router.get("/snapshot")
async def final_assurance_history_runtime_snapshot():
    runtime = (
        final_paper_assurance_history_runtime
        .status()
    )
    ...
    return {
        ...
        "assurance": {
            ...
            "scope": assurance.get("scope"),
            "assurance_score": assurance.get(
                "assurance_score"
            ),
