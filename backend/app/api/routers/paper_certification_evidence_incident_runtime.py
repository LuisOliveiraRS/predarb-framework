...
router = APIRouter(
    ...
    tags=[
        "paper-certification-evidence-incident-runtime"
    ],
)
...
@router.get("/health")
async def evidence_incident_runtime_health():
    status = paper_evidence_incident_runtime.status()
...
@router.get("/status")
async def evidence_incident_runtime_status():
...
async def evidence_incident_runtime_last_cycle():
    status = paper_evidence_incident_runtime.status()
...
@router.post("/cycle")
async def evidence_incident_runtime_cycle(
    confirm: str = Query(...),
):
    if (
        confirm
    ...
    try:
        return _safe_response(
            await paper_evidence_incident_runtime
            .capture_once()
        )
...
@router.post("/start")
async def evidence_incident_runtime_start(
    confirm: str = Query(...),
    ...
    try:
        return _safe_response(
            await paper_evidence_incident_runtime.start(
...
@router.post("/stop")
async def evidence_incident_runtime_stop(
    confirm: str = Query(...),
):
    if (
        confirm
    ...
    return _safe_response(
        await paper_evidence_incident_runtime.stop()
    )


@router.post("/reset-statistics")
async def evidence_incident_runtime_reset_statistics(
    confirm: str = Query(...),
):
    if (
        confirm
