...
class PaperAssuranceHistoryRuntime:
    """Runtime manual para persistir snapshots do Centro de Garantia Paper."""

    def __init__(
        self,
        *,
        assurance_factory: Callable[
            [],
            PaperCertificationAssuranceCenter,
        ] = PaperCertificationAssuranceCenter,
        ...
        self.enabled = (
            _env_bool(
                "PAPER_ASSURANCE_HISTORY_RUNTIME_ENABLED",
                True,
            )
            if enabled is None
            else bool(enabled)
        ...
        configured_interval = (
            _env_float(
                "PAPER_ASSURANCE_HISTORY_RUNTIME_INTERVAL_SECONDS",
                300.0,
            )
    ...
    async def capture_once(
        ...
        if not self.enabled:
            raise RuntimeError(
                "O runtime do histórico de garantia "
                "está desabilitado."
            )
